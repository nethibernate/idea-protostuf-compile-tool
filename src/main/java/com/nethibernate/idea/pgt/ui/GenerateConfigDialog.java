...
public class GenerateConfigDialog extends JDialog {
	...
	private void process() {
		...
		if (extension!=null&&EXTENSION_NAME.equals(extension)) {
			...
			//优先使用配置的protoc 路径
			if (persistentService.getConfig().getExternalToolPath() != null&&persistentService.getConfig().getExternalToolPath().endsWith("protoc")) {
				command += persistentService.getConfig().getExternalToolPath();
			} else {
				//使用 系统变量
				command += "protoc";
			}
